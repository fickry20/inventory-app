...
@section('title', 'Edit Perusahaan Tujuan - Inventory App')

@section('content')

    <!-- Page Heading -->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Perusahaan Tujuan</h1>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 bg-info">
            <h6 class="m-0 font-weight-bold text-white">Form Edit Perusahaan Tujuan: {{ $perusahaan->nama }}</h6>
        </div>
        <div class="card-body">
            <form action="{{ route('perusahaan-tujuan.update', $perusahaan->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <textarea name="alamat" id="alamat" rows="4" 
                                ...
                                placeholder="Masukkan alamat lengkap perusahaan tujuan (opsional)...">{{ old('alamat', $perusahaan->alamat) }}</textarea>
                            @error('alamat')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
